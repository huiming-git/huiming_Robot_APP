...
class RemoteAdapter {
 public:
  ...
  void set_raw_channels(const float ch[8], const uint8_t sw[4], bool failsafe, uint64_t ts_us)
  {
    diag_.frames += 1U;
    ...
    for (int i = 0; i < 4; ++i) state_.sw[i] = sw[i];
  }
 ...
 private:
  ...
  std::array<SwitchDebounce, 4> sw_{};
